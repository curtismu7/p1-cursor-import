...
class App {
  constructor() {
    ...
    const factory = (0, _apiFactory.initAPIFactory)(this.logger, this.settingsManager);
  ...
  async init() {
    try {
      ...
      this.logger.fileLogger.error('Error initializing application', {
        error: error.message
      });
    ...
    try {
      ...
      for (const [id, value] of Object.entries(fields)) {
        ...
        if (element) {
          ...
          element.value = value;
          console.log("Set ".concat(id, " to:"), value ? id.includes('secret') ? '***' : value : '(empty)');
        } else {
      ...
      if (regionSelect) {
        ...
        if (!found) {
          console.warn("Region '".concat(regionValue, "' not found in dropdown, defaulting to NorthAmerica"));
      ...
      const errorMessage = "Error populating settings form: ".concat(error.message);
      this.logger.fileLogger.error(errorMessage, {
        error: error.message
      });
  ...
  async checkSettingsAndRestore() {
    try {
      ...
      this.logger.warn("Error loading settings: ".concat(errorMessage), 'warning');
    }
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  const app = new App();
  console.log("PingOne Import Tool ".concat(app.versionManager.getFormattedVersion(), " initialized"));
...
class SettingsManager {
  ...
  async saveSettings(settings) {
    try {
      ...
      if (this.isLocalStorageAvailable()) {
        const settingsToSave = _objectSpread({}, this.settings);
      ...
      this.logger.error("Error saving settings: ".concat(error.message));
      throw error;
    }
  }
...
class UIManager {
  ...
  updateConnectionStatus(status, message) {
    ...
    console.log("Updating connection status: ".concat(status, " - ").concat(message));
  ...
  _updateStatusElement(elementId, status, message) {
    let autoHide = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;
    let element = document.getElementById(elementId);
    if (!element) {
      console.warn("Status element with ID '".concat(elementId, "' not found"));
      return;
    }
